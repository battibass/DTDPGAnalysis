...
//      root> t.Loop();       // Loop on all entries
	...
	TFile *file = new TFile("DtRpc_plots.root","recreate");
...
//       		if(!(muon_index.probe()){
// 				tag_muon.SetPtEtaPhiM(muon_index.pt.at(mu), muon_index.eta.at(mu), muon_index.phi.at(mu), 0.105);
// 				continue;
// 			}
...
// 				h_den_eta_station_1->Fill( eta_mu[0]);
// 	   			h_den_phi_station_1->Fill(phi_mu[0]);
// 	   			h_den_pt_station_1->Fill(pt_mu[0]);
				h_eff_eta_station_1->Fill(false, eta_mu[0]);
	   			h_eff_phi_station_1->Fill(false,phi_mu[0]);
	   			h_eff_pt_station_1->Fill(false,pt_mu[0]);
   				station_1_den++;
   				
	   			if(rpc_extrapolation_mb1){
// 					h_num_eta_station_1->Fill(eta_mu[0]);
// 		   			h_num_phi_station_1->Fill(phi_mu[0]);
// 	   				h_num_pt_station_1->Fill(pt_mu[0]);	
					h_eff_eta_station_1->Fill(true, eta_mu[0]);
					h_eff_phi_station_1->Fill(true, phi_mu[0]);
					h_eff_pt_station_1->Fill(true, pt_mu[0]);
	   				station_1_num++;
	   			}
		}
		if(dt_extrapolation_mb2){
// 				h_den_eta_station_2->Fill( eta_mu[0]);
// 	   			h_den_phi_station_2->Fill(phi_mu[0]);
// 	   			h_den_pt_station_2->Fill(pt_mu[0]);
				h_eff_eta_station_2->Fill(false, eta_mu[1]);
	   			h_eff_phi_station_2->Fill(false,phi_mu[1]);
	   			h_eff_pt_station_2->Fill(false,pt_mu[1]);
   				station_2_den++;
    			if(rpc_extrapolation_mb2){
// 					h_num_eta_station_1->Fill(eta_mu[1]);
// 		   			h_num_phi_station_1->Fill(phi_mu[1]);
// 	   				h_num_pt_station_1->Fill(pt_mu[1]);	
					h_eff_eta_station_2->Fill(true, eta_mu[1]);
					h_eff_phi_station_2->Fill(true, phi_mu[1]);
					h_eff_pt_station_2->Fill(true, pt_mu[1]);
	   				station_2_num++;
    			}    			
		}
	...
	TCanvas *DeltaPhi_InOut_MB1 = new TCanvas("#Delta#phi between TwinMux IN and DT Segment: MB1", "#Delta#phi between TwinMux IN and DT Segment: MB1", 210,45,750,500);
	h_DeltaPhi_InOut_MB1->GetXaxis()->SetTitle("#Delta#phi");
	h_DeltaPhi_InOut_MB1->Draw();
	
	TCanvas *DeltaPhi_InOut_MB2 = new TCanvas("#Delta#phi between TwinMux IN and DT Segment: MB2", "#Delta#phi between TwinMux IN and DT Segment: MB2", 210,45,750,500);
	h_DeltaPhi_InOut_MB2->GetXaxis()->SetTitle("#Delta#phi");
	h_DeltaPhi_InOut_MB2->Draw();
