...
TCanvas* compareEfficiencies(TString title, TEfficiency* eff1, TEfficiency* eff2, TString leg1, TString leg2)
{
  ...
  eff1->Draw(); 
  gPad->Update(); 
  auto graph_pt = eff1->GetPaintedGraph(); 
  eff1->SetMinimum(0.5);
  eff1->SetMaximum(1.03); 
  canvas->Update();
  eff2->Draw("same");   
...
void plotter()
{
  ...
  compareEfficiencies("TwinMux In eff vs #eta", h_eff_twinmux_in_eta_MB1, h_eff_twinmux_eta_pt_MB2, "MB1", "MB2");    
